...
async def run_benchmark(
    ...
    latencies.sort()
    success = sum(
        count for code, count in status_counts.items() if code and 200 <= code < 400
    )
    failures = total_requests - success

    print(f"Target: {url}")
    print(
        f"Requests: {total_requests}  Concurrency: {concurrency}  Timeout: {timeout}s"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark the Username Location Cache API."
    )
    parser.add_argument(
        "--url", default="http://localhost:8000/check", help="Full /check endpoint URL"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--usernames-file", help="Optional path to newline-delimited usernames"
    )
    ...
    async def runner():
        async with httpx.AsyncClient(
            timeout=timeout, limits=limits, transport=transport, http2=False
        ) as client:
            await run_benchmark(
                client=client,
                url=args.url,
